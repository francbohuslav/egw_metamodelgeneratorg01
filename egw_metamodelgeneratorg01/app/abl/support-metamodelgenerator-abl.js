...
/**
 * @typedef {Object} IDtoIn
 * @property {string[]} profilesFiles
 */
...
class SupportMetamodelgeneratorAbl {
  ...
  async createMetaModel(dtoIn) {
    ...
    const profiles = dtoIn.profilesFiles.map(p => JSON.parse(fs.readFileSync(p, fileReadoptions)));
    const template = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, "../", "resources/template.json"), fileReadoptions)
    );
    ...
    if (existingMetamodel) {
      ...
      res.profileList = [];
      dtoIn.mandatoryProfiles.forEach(p =>
